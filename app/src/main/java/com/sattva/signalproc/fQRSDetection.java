...
{
	public int[] fQRS(double[][] input, int Fs, int[] qrsM)
	{
		...
		int QRS1[] = FetalQRS(Channel1, Fs);
		int QRS2[] = FetalQRS(Channel2, Fs);
		int QRS3[] = FetalQRS(Channel3, Fs);
		int QRS4[] = FetalQRS(Channel4, Fs);
		
		qrsFSelectionQueue  qrsF = new qrsFSelectionQueue();
		int[] QRSF = qrsF.qrs(QRS1, QRS2, QRS3, QRS4, qrsM);
	...
	private int[] FetalQRS(double[] channel, int fs) 
	{
		...
		double Threshold = (MaxVal - MinVal)/20;
